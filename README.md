...
![Dialog 对话框](https://github.com/XiaofengdiZhu/TerrariaCraftingTableForSC/blob/main/DocRes/Dialog.webp?raw=true)  
![Crafting Recipe 合成表](https://github.com/XiaofengdiZhu/TerrariaCraftingTableForSC/blob/main/DocRes/CraftingRecipe.webp?raw=true)
...
* This mod gets crafting recipes from the `CraftingRecipesManager` one second after loading a world. So if the recipes of other mods, not in it, will not be able to craft by the Terraria Crafting Table. The following issues may appear:
...
* 本模组在每次进入存档后 1 秒时，从游戏的`CraftingRecipesManager.Recipes.Recipes`读取合成表，因此泰拉工作台无法合成未存入其中的其他模组的合成表。另外可能存在以下问题：
    * 如果其他模组在游戏进行时动态添加、修改、删除`CraftingRecipesManager.Recipes`中的合成表，本模组不会刷新合成表缓存，会继续按缓存的合成表进行合成
    * 如果其他模组确实将合成表存入了`CraftingRecipesManager.Recipes`，但添加了其他的合成限制条件，本模组不知晓也不会限制这些合成
